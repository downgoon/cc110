...
public class BanWordsManager {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(BanWordsManager.class);

	private static String banWordFilePath = "ban-words.properties";

	/** 禁词-行号 （不写文件，只加载，则用HashMap，否则用LinkedHashMap） */
	// private static transient Map<String,Integer> banWordHash = new
	...
	public static void load() {
		try {
			...
			reader = new BufferedReader(
					new FileReader(BanWordsManager.class.getClassLoader().getResource(banWordFilePath).getFile()));
			int lineNum = 0;
			...
			LOGGER.info("(Re)Load BanWord From " + banWordFilePath + " ...OK items:" + lineNum + ",cost:"
					+ ((tmEnd - tmStart) / 1000L) + "s," + (tmEnd - tmStart) + "ms");
	...
	protected static void save() {
		...
		file = new File(BanWordsManager.class.getClassLoader().getResource(banWordFilePath).getFile());
		if (!file.exists()) {
			try {
				file.createNewFile();
			} catch (IOException e) {
	...
	public static void main(String[] args) {
		String ban = BanWordsManager.hit("001工程");
		LOGGER.info("命中：" + ban);
	}
}
